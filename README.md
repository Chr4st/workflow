...
For real-time telemetry (context usage, tokens, active agents), also run: `claude plugin install claude-hud@claude-hud`. See `INSTALL.md` § 9g. For token savings on command output: `brew install rtk && rtk init -g` (60-90% savings, complements caveman). For deterministic SAST: `pip3 install semgrep` (hooks into every file write).
