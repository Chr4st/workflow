...
const { detectAndLoadMentorContext } = require('../lib/mentor-detect');
...
async function main() {
  ...
  // Load mentor context (project detection, Engram memories, GitNexus status)
  try {
    const mentorContext = await detectAndLoadMentorContext(process.cwd());
    if (mentorContext) {
      output(mentorContext);
