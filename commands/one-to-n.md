...
- **B34** — `Agent(database-reviewer)` THIRD — only if the change touches schema, queries, or migrations. Receives cumulative B32 + B33 findings. For query-heavy changes, pair this with the `database-migrations` skill so the reviewer has the migration context in scope. If B32 or B33 flagged CRITICAL findings or hedging language, escalate this reviewer from Haiku to Sonnet.
