...
27. **A27 — Language-specific reviewer (go-reviewer / python-reviewer as applicable)** — runs SECOND. Receives A25.5 context brief + A26 security findings. Adds language-specific depth that /orchestrate doesn't cover. Skip if no language-specific reviewer exists for the project's stack. Use **Haiku 4.5** model.
