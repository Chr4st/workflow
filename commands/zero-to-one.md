...
If ast-grep MCP is available, use it to check candidate skeletons for structural anti-patterns (e.g., `eval($$$)`, `innerHTML = $USER_INPUT`, unsanitized query construction).
...
21.5. **A21.5 — Property-based testing.** After unit tests pass GREEN, generate property-based tests using `hypothesis` (Python), `fast-check` (TS/JS), `gopter` (Go), `proptest` (Rust). Properties define invariants (e.g., 'serialization round-trips are identity', 'for all valid inputs, output satisfies postcondition'). Run them. Failures = RED. Catches 37.3% more bugs than example-based tests (arXiv:2506.18315).
22. **A22 — Implement minimal GREEN code**. Use **Sonnet 4.6** as the implementation model (best coding model per `performance.md`). Constraints from `coding-style.md`: files under 150 LOC, functions under 50 lines, no nesting deeper than 4, immutable data, no hardcoded values. Extract utilities aggressively.
...
24. **A24 — Lint feedback loop.** Run linter + type-checker on changed files. If violations: feed specific messages back, auto-fix, re-verify. Cap at 2 iterations (+24% per ICSME 2025). After lint-clean, `/verify quick` (build + types) as final checkpoint.
...
25.5. **A25.5 — Compute context brief for first reviewer.** Apply AgentDiet trajectory compression before computing the brief: classify all Phase 1–4 outputs as KEEP (findings, decisions, test results), SUMMARIZE (verbose command output → 1–2 line summary), or DROP (expired state, superseded drafts). The resulting brief should be 40–60% smaller than raw Phase 1–4 output. Each reviewer runs as its own sub-spec Agent. Pass only KEEP outputs as context brief — do not forward raw conversation history. Before launching reviewers, the orchestrator computes a structured brief and passes it to the FIRST reviewer (security, A26). The brief contains: (1) list of changed files with line counts, (2) gitnexus impact results from any prior impact analysis, (3) test status (pass/fail/coverage), (4) /orchestrate feature findings summary, (5) stack and framework detected. This prevents each reviewer from independently re-reading the codebase and re-deriving project structure — per Latent Briefing research, this saves 42–57% of worker tokens. **Prompt Cache Rule:** Pass this brief as a user message to the first reviewer, NOT as part of the system prompt. Do not change the tool set between A26, A27, A28, and A29 — changing tools invalidates the prompt cache prefix.
26. **A26 — `Agent(security-reviewer)`** — runs FIRST. OWASP top-10, secrets scan, injection review, authn/authz surface, rate limiting, error-message leakage. Per `security.md` mandatory checklist. Use **Sonnet 4.6** model. Security findings have highest blast radius and must inform all subsequent reviewers.
NOTE: Mechanical OWASP pattern matching (injection, XSS, secrets, unsafe deserialization) is handled by the semgrep PostToolUse hook at write time. Focus on architectural security: auth flows, session management, crypto design, trust boundaries.
After A26 completes, check its confidence marker (see performance.md Model Routing). Each reviewer MUST end its output with `[CONFIDENCE: HIGH]`, `[CONFIDENCE: MEDIUM]`, or `[CONFIDENCE: LOW]`. If A26 reports `[CONFIDENCE: LOW]` or found CRITICAL issues, escalate A27 from Haiku 4.5 to Sonnet 4.6.
27. **A27 — Language-specific reviewer (go-reviewer / python-reviewer as applicable)** — runs SECOND. Receives A25.5 context brief + A26 security findings. Adds language-specific depth that /orchestrate doesn't cover. Skip if no language-specific reviewer exists for the project's stack. Use **Haiku 4.5 unless escalated by A26 findings (see escalation rules in performance.md)**.
28. **A28 — `Agent(database-reviewer)`** _(conditional)_ — runs THIRD. Receives cumulative A26+A27 findings. If the project uses persistence (SQL, NoSQL, ORM, file DB), review schema, indexes, migration strategy, N+1 query patterns. Skip entirely if no persistence layer.
Model escalation applies — check A26 and A27 confidence markers and findings before selecting model tier.
28.5. **A28.5 — Compile prioritized finding list.** Sequential findings already chained. Compile prioritized list of all findings from A26–A28, deduplicate (same file + same issue = one finding), and present one unified review digest to the user — not three separate reports.
...
30.5. **A30.5 — Mutation testing.** After coverage clears 80% in A30, run mutation testing on changed files only: `npx stryker run` (JS/TS) or `mutmut run` (Python) or language equivalent. Target: 60%+ mutation score on changed files. If below 60%, strengthen tests at weakest seams before Phase 6. Runtime target: under 5 minutes. On timeout: skip, note in PR body, proceed.
...
- **Parallelization opportunities:**
  ...
  - Phase 5: Reviewers run sequentially (A26→A27→A28). Each sees prior findings. arXiv:2511.02309.
...
- **File-size hygiene:** flag at write time any file crossing 150 LOC (per `coding-style.md` and `mentor.md`). Suggest extraction to a separate module immediately — do not defer.
