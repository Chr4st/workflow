...
6. **C6.** `mcp__gitnexus__context <suspect symbol>` — fetch the knowledge-graph neighborhood (related types, files, tests). Use ast-grep for structural pattern matching on the suspect symbol's usage — catches indirect references and pattern-based dispatch that gitnexus may not index.
...
17.5. **C17.5 — Mutation testing (CONDITIONAL).** Run only if the fix touched >3 files or if the locked test from C11 is the ONLY new test. If triggered: run mutation testing on changed files to verify the locked test is not trivially killable. Target: 60%+ mutation score on the fix diff. Skip for single-file, single-test fixes where the locked test already has strong assertions.
...
22.5. **C22.5.** **Compute context brief for reviewers.** Apply AgentDiet trajectory compression before computing the brief: classify Phase 1–4 outputs as KEEP (root cause, error text, locked test assertion, fix diff), SUMMARIZE (explore outputs and gitnexus context → key findings), DROP (verbose command output, file listings, superseded hypotheses). Target 40–60% reduction. Each reviewer runs as its own sub-spec Agent. Pass only KEEP outputs as context brief — do not forward raw conversation history. Before launching reviewers, compute: (1) the bug root cause (one sentence), (2) files changed with diff summary, (3) the locked test from C11 and its assertion, (4) `/verify` results from C16. Pass this brief forward through the reviewer chain so each reviewer builds on prior findings instead of re-investigating the bug. **Prompt Cache Rule:** Pass this brief as a user message, not system prompt content. Maintain the same tool set across C23, C24, and C25 to preserve prompt cache prefix hits.
23. **C23.** `Agent(security-reviewer)` or skill `security-review` — **CONDITIONAL, runs FIRST.** Sonnet 4.6. Run only if the bug or fix touches auth, crypto, input validation, secrets, or error messages. For logic bugs, perf fixes, or UI-only bugs, skip and note "security review skipped: no security surface." When run, its findings feed into C24. NOTE: Mechanical OWASP pattern matching is handled by the semgrep PostToolUse hook at write time. Focus on architectural security: auth flows, session management, crypto design, trust boundaries. After C23 completes, check its confidence marker (see performance.md Model Routing). Each reviewer MUST end its output with `[CONFIDENCE: HIGH]`, `[CONFIDENCE: MEDIUM]`, or `[CONFIDENCE: LOW]`. If C23 reports `[CONFIDENCE: LOW]` or found CRITICAL issues, escalate C24 to Sonnet 4.6.
24. **C24.** Run `Agent(code-reviewer)` or `/code-review` (skill `code-review`). **Runs SECOND.** Receives the context brief from C22.5 plus any security findings from C23. Mandatory per `CLAUDE.md`. Look for neighboring breakage: call sites you didn't notice, shared state you mutated, error paths you silenced. Model escalation: if C23 reported `[CONFIDENCE: LOW]` or found CRITICAL issues, use Sonnet 4.6. Otherwise use base routing.
25. **C25.** Run the language-specific reviewer **THIRD.** Haiku 4.5. Receives cumulative findings from C23 and C24. Model escalation: if C23 reported `[CONFIDENCE: LOW]` or found CRITICAL issues, escalate to Sonnet 4.6. Otherwise use Haiku 4.5 base routing. Pick the right reviewer:
